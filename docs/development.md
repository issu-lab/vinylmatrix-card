...
Open `http://127.0.0.1:5177`. The preview uses the generated bundle and two simulated players. It has no connection to Home Assistant. Rebuild after source changes. The only dependency allowed to run an installation build script is esbuild.
...
src/styles.ts            four responsive themes
src/graphics.ts          native SVG tonearm and icons
