...
export class VinylMatrixEditor extends LitElement {
  ...
  protected render() {
    ...
    return html`
      ...
      <label for="theme">${t.theme}</label><select id="theme" .value=${this.config.theme ?? "minimal"} @change=${(e:Event)=>this.updateConfig({ theme:(e.target as HTMLSelectElement).value as Config["theme"] })}>${["minimal","classic"].map(v=>html`<option value=${v}>${t[v as keyof typeof t]}</option>`)}</select>
